...
with tab_summary:
    ...
    st.caption(f"Prepared by: {student_name}")
    ...
    with dl2:
        excel_buf = build_excel_report(companies, benchmark_sector, benchmark, matrix_rows, prepared_by=student_name)
        st.download_button("Excel (.xlsx)", data=excel_buf, file_name="company_ratio_comparison.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            use_container_width=True)
    with dl3:
        pdf_buf = build_pdf_report(companies, benchmark_sector, matrix_rows, prepared_by=student_name)
