...
def build_ratio_prompt(companies, benchmark_sector, matrix_rows):
    ...
    lines += [
        ...
        "",
        "If (and only if) you have web search available, you may add ONE final short "
...
def _extract_grounding_sources(response):
    """Pulls the (title, url) pairs Gemini actually cited when it used Google Search,
    so students can see and check the real sources behind any 'Recent context' section."""
    sources = []
    try:
...
with st.sidebar:
    ...
    ai_use_search = st.checkbox(
        ...
        value=False,
        help="Adds an optional 'Recent context' blurb with real, cited web sources — "
             "the ratio numbers themselves always come from your data, never from search. "
...
with tab_summary:
    ...
    if "ai_analysis_text" in st.session_state:
        ...
        if st.session_state.get("ai_analysis_sources"):
            with st.expander("🔍 Web sources Gemini cited for the 'Recent context' section"):
                for s in st.session_state["ai_analysis_sources"]:
